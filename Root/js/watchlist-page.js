...
export async function renderLibraryPage(appRoot, params = {}) {
    const { activeFilter = 'watchlist' } = params;
    ...
    const tabsHtml = `
        <div class="media-tabs library-media-tabs">
            <button class="media-tab active" data-media-type="all">All</button>
            <button class="media-tab" data-media-type="movie">Films</button>
            <button class="media-tab" data-media-type="tv">TV Series</button>
        </div>
    `;

    const itemsOnLists = Array.from(watchlistMap.values()).filter(item => ['watchlist', 'watched', 'not_interested'].includes(item.status));
