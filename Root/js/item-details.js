...
function populateOverlay(card, data, mediaType) {
    ...
    if (countryName) {
        detailsHtml += `<li><i class="fa-solid fa-earth-americas"></i> ${countryName}</li>`;
    }

    if (data.original_language) {
        const lang = new Intl.DisplayNames(['en'], { type: 'language' }).of(data.original_language);
        if(lang) detailsHtml += `<li><i class="fa-regular fa-message"></i> ${lang}</li>`;
    }
    if (data.genres && data.genres.length > 0) {
        detailsHtml += `<li><i class="fa-solid fa-clapperboard"></i> ${data.genres.map(g => g.name).slice(0, 2).join(', ')}</li>`;
    }
    ...
    if (mediaType === 'tv') {
        ...
        if (data.networks && data.networks.length > 0) {
            detailsHtml += `<li><i class="fa-solid fa-tv"></i> ${data.networks[0].name}</li>`;
        }
    }
    ...
    if (mediaType === 'tv' && data.status) {
        ...
        if (statusClass) {
            statusTagPlaceholder.innerHTML = `<div class="status-tag ${statusClass}">${statusText}</div>`;
        }
    }
}
...
async function handleWatchlistActionFeedback(card, mediaId, mediaType, newStatus, previousStatus) {
    ...
    // Case 1: Library page - re-render the whole thing
    if (isLibraryPage) {
        if (card) {
            ...
            setTimeout(async () => {
                ...
                renderLibraryPage(appRoot, { activeFilter: currentActiveFilter });
            }, 300);
        }
    ...
    if (newStatus !== 'remove' && card) {
        ...
        toastOptions.onUndo = async () => {
            ...
            if (undoSuccess) {
                ...
                // The UI will be out of sync (the card is gone), but the data is correct.
                // A page refresh would fix it, but this is less disruptive.
                const { handleRouteChange } = await import('./router.js');
                handleRouteChange();
            } else {
