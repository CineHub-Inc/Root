...
async function fetchAndAppendFilm() {
    if (replacementPool.length === 0) {
        ...
        try {
            ...
            console.error("Failed to fetch replacement films:", e);
        } finally {
    ...
    if (newItem) {
        ...
        setTimeout(fetchAndAppendFilm, 100);
    }
}

function handleFilmReplacementRequest(event) {
    if (event.detail.gridId === 'films-grid') {
        fetchAndAppendFilm();
    }
}
    ...
    try {
        ...
        if (data && data.results) {
            ...
            hasMore = false;
             if (!append) {
                const grid = document.getElementById('films-grid');
                if (grid) grid.innerHTML = `<p class="no-content-message">No films match the selected criteria.</p>`;
            }
        }
    } catch (error) {
...
function handleFilterChange(newFilters) {
    ...
    loadFilms(1, false); // Fetch page 1, don't append
...
/**
 * Renders the entire Films page, including the filterable grid.
 * @param {HTMLElement} appRoot - The main application container.
 * @param {object} params - The URL query parameters.
 */
export function renderFilmsPage(appRoot, params) {
    const prefs = getExplicitPreferences();
    const prefFilters = {};
    ...
    currentFilters = {
        ...
        'vote_count.gte': 200,
        'primary_release_date.lte': today, // Exclude upcoming films
        ...prefFilters
    };
    ...
    appRoot.innerHTML = `
        <div class="page-header">
            <h1 class="shelf-title">Films</h1>
        </div>
        <div class="media-grid" id="films-grid"></div>
    ...
    appRoot.removeEventListener('request-grid-replacement', handleFilmReplacementRequest);
    appRoot.addEventListener('request-grid-replacement', handleFilmReplacementRequest);

    loadFilms(randomPage).then(() => {
        setupInfiniteScroll();
