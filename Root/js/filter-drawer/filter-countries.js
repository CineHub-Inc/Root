...
export function render(mediaType) {
    return `
        <section class="filter-section" data-filter-type="countries">
            <div class="filter-section-header">
                <h3>Country</h3>
            </div>
            <div class="select-container">
                <button class="select-btn" aria-haspopup="listbox">
                    <span class="select-btn-text">Any Country</span>
                    <i class="fas fa-chevron-down"></i>
                </button>
                <input type="text" class="select-search-input" placeholder="Search countries...">
...
export async function init(sectionElement, mediaType) {
    ...
    try {
        optionsData = await getCountries();
        if (optionsData && optionsData.length > 0) {
            optionsData.sort((a, b) => a.english_name.localeCompare(b.english_name));
            const optionsHtml = optionsData.map(c => 
                `<li role="option" data-value="${c.iso_3166_1}">${c.english_name}</li>`
            ).join('');
            optionsEl.innerHTML = optionsHtml;
        } else {
            optionsEl.innerHTML = '<li>No countries available</li>';
        }
    } catch (error) {
        console.error("Failed to load countries:", error);
        optionsEl.innerHTML = '<li>Could not load countries</li>';
    }
}

export function getValue() {
    if (!containerEl || !containerEl.dataset.selectedValue) return {};
    return { with_origin_country: containerEl.dataset.selectedValue };
}

export function reset() {
    if (!containerEl) return;
    delete containerEl.dataset.selectedValue;
    btnTextEl.textContent = 'Any Country';
