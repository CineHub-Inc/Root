...
const WATCH_BASE_URL = 'https://vsrc.su/embed/';
...
export function isWatchFeatureUnlocked() {
    ...
    if (userData && userData.powerUserExpiresAt) {
        ...
        const expiryDate = userData.powerUserExpiresAt.toDate();
...
export function watchMovie(tmdbId) {
    ...
    const url = `${WATCH_BASE_URL}movie/${tmdbId}`;
...
export async function watchTvShow(tmdbId, seasonNumber, episodeNumber) {
    ...
    const url = `${WATCH_BASE_URL}tv/${tmdbId}/${seasonNumber}/${episodeNumber}`;
    const newTab = window.open(url, '_blank');
