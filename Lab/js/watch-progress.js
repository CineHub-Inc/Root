...
/**
 * Checks if a series is complete and updates its main status to 'watched' or 'caught-up'.
...
async function checkAndMarkSeriesAsWatched(seriesId) {
    try {
        ...
        if (currentStatus === 'watched') {
            return; // Already marked as fully watched, no action needed.
        }

        // Calculate total aired episodes (excluding specials)
        let totalAired = 0;
        const lastEp = details.last_episode_to_air;
        
        if (lastEp && lastEp.season_number > 0) {
            details.seasons.forEach(season => {
                if (season.season_number === 0) return;
                if (season.season_number < lastEp.season_number) {
                    totalAired += season.episode_count;
                } else if (season.season_number === lastEp.season_number) {
                    totalAired += lastEp.episode_number;
                }
            });
        } else {
            // Fallback if last_episode_to_air is missing or is a special
            totalAired = getTotalSeriesEpisodes(details.seasons);
        }

        // Calculate watched count (excluding specials)
        const progress = getWatchedProgress();
        const seriesProgress = progress[seriesId] || {};
        let watchedCount = 0;
        for (const seasonKey in seriesProgress) {
            if (seasonKey !== 'lastWatchedTimestamp' && seasonKey !== 's0') {
                watchedCount += (seriesProgress[seasonKey] || []).length;
            }
        }

        if (totalAired > 0 && watchedCount >= totalAired) {
            const seriesStatus = details.status;
            if (seriesStatus === 'Returning Series' || seriesStatus === 'On Air' || seriesStatus === 'In Production' || seriesStatus === 'Planned') {
                // This is an ongoing series, and the user is caught up.
                await updateItemStatus(seriesId, 'tv', 'caught-up');
                showToast({ message: `You're all caught up on ${details.name}!`, type: 'info' });
            } else { // 'Ended', 'Canceled', etc.
                // This is a finished series.
                await updateItemStatus(seriesId, 'tv', 'watched');
            }
        }
    } catch (error) {
