...
async function loadAndPopulateDetails(item) {
    ...
    try {
        if (item.media_type === 'movie' || item.media_type === 'tv') {
            ...
            detailsElement.innerHTML = [escapeHTML(year), escapeHTML(country)].filter(Boolean).join(' &bull; ');
            ...
            detailsElement.innerHTML = [escapeHTML(country), escapeHTML(ageText)].filter(Boolean).join(' &bull; ');
        }
    } catch (error) {
...
function renderResults(results) {
    ...
    const createItemHtml = (item) => {
        ...
        if (item.media_type === 'movie' || item.media_type === 'tv' || (item.media_type === 'person' && (item.known_for_department === 'Acting' || item.known_for_department === 'Directing'))) {
            ...
            detailsHtml = `<span class="result-type">${escapeHTML(typeDisplay)}</span>`;
        }
    ...
    const createCategoryHtml = (title, items) => {
        ...
        return `
            <div class="search-category">
                <h3 class="search-category-title">${escapeHTML(title)}</h3>
